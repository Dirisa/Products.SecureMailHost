...
# RCS-ID:       $Id: PloneFormMailer.py,v 1.14 2004/10/20 21:11:10 dreamcatcher Exp $
...
class PloneFormMailer(BaseContent, TALESLines, TALESString):
    ...
    def __init__(self, oid, **kwargs):
        ...
        self.form.manage_addField('fullname', 'Full Name', 'StringField')
        self.form.manage_addField('email', 'e-mail', 'EmailField')
        self.form.manage_addField('message', 'Your Message', 'TextAreaField')
    ...
    def setBodyPT(self, value, **kwargs):
    ...
    def getButtons(self):
        buttons=[]
        for buttondef in self.getFormButtons():
            button= []
    ...
    def get_mail_body(self, prepend='', append='', hidefields=[], **kwargs):
        ...
        encryption = encryption and keyid
        ...
        if encryption:
            ...
            # else
            # print "!!! PloneFormMailer",self.id,(": gpg encryption failed. "
            #                                      "A key is given but invalid.")
            # if not failed
    ...
    def get_header_body_tuple(self, prepend='', append='',
        ...
        recip_name = self.getEvaluatedExpression('recipient_name')
        recip_email = self.getEvaluatedExpression('recipient_email')
        ...
        headerinfo = {
            ...
            'From': from_addr,
            'Subject': subject,
            'Content-Type': '%s; charset=%s' % (self.getBodyType() or 'text/html',
                                                self._site_encoding()),
            'User-Agent': 'PloneFormMailer for Zope/Plone by jensquadrat.com'}

        # CC
        cc_recips = self.getEvaluatedExpressions('cc_recipients')
        addrs = ['<%s>' % addr for addr in cc_recips]
        headerinfo['Cc'] = ', '.join(addrs)

        # BCC
        bcc_recips = self.getEvaluatedExpressions('bcc_recipients')
        addrs = ['<%s>' % addr for addr in bcc_recips]
        headerinfo['Bcc'] = ', '.join(addrs)
        headerinfo['MIME-Version'] = '1.0'

        # return 3-Tuple
        return (headerinfo, self.getAdditionalHeaders(), body)
...
InitializeClass(PloneFormMailer)
