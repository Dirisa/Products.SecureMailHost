...
class Node(object, Common):
	...
	def to_string(self):
		...
		# RMF: special case defaults for node and edge properties.
...
class Edge(object, Common):
	...
	def to_string(self):
		...
		if	self.parent_graph and		\
			self.parent_graph.type and	\
			self.parent_graph.type=='digraph':
			edge+=' -> '
		else:
			edge+=' -- '
		edge+=dst
...
class Graph(object, Common):
	...
	attributes=['Damping', 'bb', 'center', 'clusterrank', 'compound', 'concentrate',\
		...
		'label' ,'fontname', 'comment', 'lp', 'target']
	...
	def get_node(self, name):
		...
		match=[node for node in self.node_list if node.name == name]
...
class Dot(Graph):
	...
	def __init__(self, **args):
		...
		for frmt in self.formats:
			self.__setattr__('create_'+frmt, lambda prog='dot', f=frmt : self.create(prog, f))
			f=self.__dict__['create_'+frmt]
			f.__doc__='''Refer to docstring from 'create' for more information.'''
			
		for frmt in self.formats+['raw']:
			self.__setattr__('write_'+frmt, lambda path, prog='dot', f=frmt : self.write(path, prog, f))
